...
**new version**: edited by Fatemeh Hoseini
...
1. Train:  
  ```python legged_gym/scripts/train.py --task=anymal_c_flat```
    ...
    To run on GPU add following arguments: `--sim_device=cuda`, `--rl_device=cuda`, `--pipeline=cpu`
...
```python legged_gym/scripts/play.py --task=iust```
